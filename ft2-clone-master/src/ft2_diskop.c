...
bool setupDiskOp(void)
{
	...
	strcpy(trkTmpFName, "untitled.amds");
...
{
	textOutShadow(19, 101, PAL_FORGRND, PAL_DSKTOP2, "AMDS");	// One format, sorry :3
}
...
static void setDiskOpItemRadioButtons(void)
{
	...
	if (FReq_Item == DISKOP_ITEM_TRACK || FReq_Item == DISKOP_ITEM_SONG)   radioButtons[RB_DISKOP_TRK_SAVEAS_XT].state = RADIOBUTTON_CHECKED;

	if (ui.diskOpShown)
		showRadioButtonGroup(RB_GROUP_DISKOP_TRK_SAVEAS);	// Useless to give argument as it always shows the same only option
}
...
static void drawDiskOpScreen(void)
{
	...
	FReq_Item = DISKOP_ITEM_SONG; // Select the first one
	
	uncheckRadioButtonGroup(RB_GROUP_DISKOP_ITEM);
	radioButtons[RB_DISKOP_SONG + FReq_Item].state = RADIOBUTTON_CHECKED;
	showRadioButtonGroup(RB_GROUP_DISKOP_ITEM);

	// item selector (for Amadeus we either save the track [part of a song] or the song)
	textOutShadow(5,   3, PAL_FORGRND, PAL_DSKTOP2, "Item:");
	textOutShadow(19, 17, PAL_FORGRND, PAL_DSKTOP2, "Song");
	textOutShadow(19, 31, PAL_FORGRND, PAL_DSKTOP2, "Track");
...
void rbDiskOpSong(void)
{
	checkRadioButton(RB_DISKOP_SONG);
	setDiskOpItem(DISKOP_ITEM_SONG);
}
