...
{
#pragma region NotTheNotes
	if (ui.sysReqShown || editor.editTextFlag)
		return;
		
	if (ui.extended)
	{
		if (mouse.y <= 52) // Are we in the upper zone ?
		{
		...
		{
			directionUp ? mouseWheelDecRow() : mouseWheelIncRow();	// Actually move around the notes
		}
...
#pragma endregion NotTheNotes
		else if (ui.patternEditorShown)
		{
			recordNote(0, 0, false);	// Reset the nextMove counter
			directionUp ? mouseWheelDecRow() : mouseWheelIncRow();	// Actually move the notes
		}
	}
}
