...
int8_t scancodeKeyToNote(SDL_Scancode scancode)
{
	...
	// If we are here, it's a normal key
	if (modState & KMOD_NUM && scancode >= SDL_SCANCODE_KP_1 && scancode <= SDL_SCANCODE_KP_0) // If numpad is in number mode and it presses a number => return the number
		return (scancode - SDL_SCANCODE_KP_1 + 1) % 10;

	if (scancode >= SDL_SCANCODE_1 && scancode <= SDL_SCANCODE_0) // If he pushes a number, put number
		return (scancode - SDL_SCANCODE_1 + 1) % 10;
...
#pragma endregion NOPE

	if (handleEditKeys(keycode, scancode))	// Real deal
		return;
...
static void handleKeys(SDL_Keycode keycode, SDL_Scancode scanKey)
{
	...
	switch (keycode)
	{
		...
		case SDLK_LEFT:  recordNote(0, 0, false); cursorLeft();  break;
		case SDLK_RIGHT: recordNote(0, 0, false); cursorRight(); break;
